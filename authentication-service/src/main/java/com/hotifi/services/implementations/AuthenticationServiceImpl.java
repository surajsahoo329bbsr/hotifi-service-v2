...
import com.hotifi.exception.errors.AuthenticationErrorMessages;
...
public class AuthenticationServiceImpl implements IAuthenticationService {
    ...
    public Authentication getAuthentication(String email) {
        ...
        if (authentication == null || isAdminEmail)
            throw new AppException(AuthenticationErrorCodes.EMAIL_NOT_FOUND);
    ...
    public CredentialsResponse addEmail(String email, String identifier, String token, String socialClient) {
        ...
        if (socialAddEmail && !verificationService.isSocialUserVerified(email, identifier, token, SocialCodes.valueOf(socialClient)))
            throw new AppException(AuthenticationErrorCodes.USER_SOCIAL_IDENTIFIER_INVALID);
        try {
            ...
            throw new AppException(AuthenticationErrorCodes.EMAIL_ALREADY_EXISTS);
        } catch (Exception e) {
            log.error("Error Occurred ", e);
            throw new AppException(AuthenticationErrorCodes.UNEXPECTED_AUTHENTICATION_ERROR);
        }
    }
    ...
    public void resendEmailOtpSignUp(String email) {
        ...
        if (authentication.isEmailVerified())
            throw new AppException(AuthenticationErrorCodes.EMAIL_ALREADY_VERIFIED);
    ...
    public void verifyEmailOtpSignUp(String email, String emailOtp) {
        ...
        if (BCrypt.checkpw(emailOtp, encryptedEmailOtp)) {
            ...
            throw new AppException(AuthenticationErrorCodes.EMAIL_OTP_INVALID);
        }
    }
    ...
    public void verifyPhone(String email, String countryCode, String phone, String token) {
        ...
        if (authentication == null)
            throw new AppException(AuthenticationErrorCodes.EMAIL_NOT_FOUND);
        if (!authentication.isEmailVerified())
            throw new AppException(AuthenticationErrorCodes.EMAIL_NOT_VERIFIED);
        if (!verificationService.isPhoneUserVerified(countryCode, phone, token, CloudClientCodes.GOOGLE_CLOUD_PLATFORM))
            throw new AppException(AuthenticationErrorCodes.PHONE_TOKEN_INVALID);
        try {
            ...
            throw new AppException(AuthenticationErrorCodes.PHONE_ALREADY_EXISTS);
        } catch (Exception e) {
            throw new AppException(AuthenticationErrorCodes.UNEXPECTED_AUTHENTICATION_ERROR);
        }
    }
